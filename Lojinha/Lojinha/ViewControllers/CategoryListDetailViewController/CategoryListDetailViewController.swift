...
class CategoryListDetailViewController: ControlCacheViewController, VisibleView
{
    ...
    var listProductsViewModel: ProductViewMode?
    {
        didSet
        {
            listProductsViewModel?.listProductDidChange = { [weak self] viewModel in
                ...
                {
                    self?.present(Alerts.showAlert(message: "Não há produtos disponíveis agora..."), animated: true, completion: nil)
    ...
    override func viewDidLoad()
    {
        ...
        self.setTitle(title: category.descriptions)
    ...
    {
        let url = ApiURL.get(type: .productByCategory, offset: String(offSet)) + String(category.idCategoria)
        listProductsViewModel?.getElement(withURL: url, completion: { (error) in
            if let error = error
            {
                self.isView(hide: true)
                print("-> Error get product by category: \(error)")
    ...
    private func infinityScroll()
    {
        categoryListDetailTableView.addInfiniteScroll { (tableView) in
            if self.listProductsViewModel?.list?.products.count ?? 0 == Int(CountProductsList.range.rawValue) ?? 0
            {
                let offset = (self.listProductsViewModel?.list?.offSet ?? 0) + Int(CountProductsList.range.rawValue)!
