...
class HomeViewController: ControlCacheViewController, VisibleView
{
    ...
    private func getBanners()
    {
        listBannerViewModel?.getElement(completion: { (error) in
            if let error = error
            {
                print("-> Error get banners [ViewController]: \(error)")
    ...
    private func getCategories()
    {
        listCategoryViewModel?.getElement(completion: { (error) in
            if let error = error
            {
                print("-> Error get categories: \(error)")
                self.present(Alerts.showAlert(message: error.localizedDescription), animated: true, completion: nil)
            }
        })
    }
    ...
    private func getTopSellingProducts()
    {
        listTopSellingProducts?.getElement(completion: { (error) in
            if let error = error
            {
                print("-> Error get top selling products: \(error)")
                self.present(Alerts.showAlert(message: error.localizedDescription), animated: true, completion: nil)
            }
        })
    }
